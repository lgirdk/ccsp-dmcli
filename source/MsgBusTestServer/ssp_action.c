...
#define  CCSP_DATAMODEL_XML_FILE           "/usr/ccsp/MsgBusTestServer/MsgBusTest.XML"
    ...
    )
{
    /* Create component common data model object */
     ...
     g_pComponent_COMMON_testserver = (PCOMPONENT_COMMON_TESTSERVER)AnscAllocateMemory(sizeof(COMPONENT_COMMON_TESTSERVER));

    if ( ! g_pComponent_COMMON_testserver )
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit( g_pComponent_COMMON_testserver);

     g_pComponent_COMMON_testserver->Name     = AnscCloneString(CCSP_COMPONENT_NAME_TESTSERVER);
     g_pComponent_COMMON_testserver->Version  = 1;
     g_pComponent_COMMON_testserver->Author   = AnscCloneString("Your name");
    ...
    )
{
    ...
    char                            CrName[256];
     ...
     g_pComponent_COMMON_testserver->Health = CCSP_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS || returnStatus == CCSP_SUCCESS )
    {
        /* System is fully initialized */
         g_pComponent_COMMON_testserver->Health = CCSP_COMMON_COMPONENT_HEALTH_Green;
    }
	else
	{
    ...
    )
{
    ...
    if(  g_pComponent_COMMON_testserver == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        _ansc_sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s%s", g_Subsystem, CCSP_COMPONENT_NAME_TESTSERVER);
    }
    else
    {
        _ansc_sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        _ansc_sprintf(CpName, "%s", CCSP_COMPONENT_NAME_TESTSERVER);
    }
    ...
    if (  g_pComponent_COMMON_testserver ) AnscFreeMemory( g_pComponent_COMMON_testserver);

     g_pComponent_COMMON_testserver = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponent_COMMON_testserver->Name;
}
...
{
    return  g_pComponent_COMMON_testserver->Version;
}
...
{
    return  g_pComponent_COMMON_testserver->Author;
}
...
{
    return  g_pComponent_COMMON_testserver->Health;
}
...
{
    return  g_pComponent_COMMON_testserver->State;
}
...
{
    return  g_pComponent_COMMON_testserver->LogEnable;
}
...
{
    if( g_pComponent_COMMON_testserver->LogEnable == bEnabled) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_testserver->LogEnable = bEnabled;
    if(bEnabled) g_iTraceLevel = (INT)  g_pComponent_COMMON_testserver->LogLevel;
...
{
    return  g_pComponent_COMMON_testserver->LogLevel;
}
...
{
    if( g_pComponent_COMMON_testserver->LogLevel == LogLevel) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_testserver->LogLevel = LogLevel;
    if( g_pComponent_COMMON_testserver->LogEnable) g_iTraceLevel = (INT)  g_pComponent_COMMON_testserver->LogLevel;
...
{
    return  g_pComponent_COMMON_testserver->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(CCSP_COMPONENT_NAME_TESTSERVER);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponent_COMMON_testserver->LogLevel); */
